...
st.title("Stacked Block View by Area (EXE)")
...
if uploaded_file is not None:
    ...
    # Filter Area yang mau ditampilkan
    ...
    for area in area_order:
        ...
        for y_offset, rowbay in enumerate(rowbays):
            ...
            for i, row in enumerate(stacks.itertuples()):
                color = get_color(row.Move, row._4)
                ax.add_patch(
                    mpatches.Rectangle(
    ...
    st.info("Silakan upload file Excel berisi kolom: Area, Row_Bay, Move, Carrier Out")
