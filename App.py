...
if uploaded_file:
    ...
    areas_to_show = [f"{l}{i:02}" for i in range(8, 0, -1) for l in ['C', 'B', 'A']]
    ...
    def get_color(move, carrier):
        nonlocal_color_index = get_color.__dict__.setdefault('color_index', 0)
        if move == 'Import':
            return 'lightgrey'
        if carrier not in carrier_colors:
            carrier_colors[carrier] = color_palette[nonlocal_color_index % len(color_palette)]
            get_color.__dict__['color_index'] += 1
    ...
    st.info("Silakan upload file Excel yang berisi data Area, Row_Bay, Move, dan Carrier Out.")
